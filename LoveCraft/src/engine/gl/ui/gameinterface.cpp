...
void GameInterface::TextUpdate()
{
	...
	ss.str("");
	//Energie
	ss << (int)m_val->Character->Energy() << " / " << (int)m_val->Character->EnergyMax();
	m_lbl_energy->SetMessage(ss.str());
	ss.str("");
	//Mana
	ss << (int)m_val->Character->Mana() << " / " << (int)m_val->Character->ManaMax();
	m_lbl_mana->SetMessage(ss.str());
	ss.str("");
	//Experience
	ss << (int)m_val->Character->Exp() << " / " << (int)m_val->Character->ExpNext();
	m_lbl_exp->SetMessage(ss.str());
	ss.str("");
	//Niveau
	ss << m_val->Character->Level();
	m_lbl_playerLevel->SetMessage(ss.str());
	ss.str("");
	//Position
	ss << "Position :     ( " << std::setprecision(4) << m_val->Player->Position().x << ", " << std::setprecision(4) <<
		m_val->Player->Position().y << ", " << std::setprecision(4) << m_val->Player->Position().z << " )";
	m_lbl_plrPos->SetMessage(ss.str());
	ss.str("");
	//Vitesse
	ss << "Vitesse :      " << m_val->Player->Speed();
	m_lbl_plrSpd->SetMessage(ss.str());
	ss.str("");
	//Accélération
	ss << "Acceleration : " << m_val->Player->Acceleration();
	m_lbl_plrAcc->SetMessage(ss.str());
	ss.str("");
	//Psotion souris
	ss << "Pos. Souris :  " << m_val->Mousepos;
	m_lbl_mousePos->SetMessage(ss.str());
	ss.str("");
	//FPS
	ss << "Fps :          " << std::setprecision(2) << m_val->Fps;
...
{
	// Écran
	m_pnl_screen = new Panel(0, Vector2i(), Vector2i(m_val->Width, m_val->Height), 0, 1, "main");
...
#pragma region Enfants pnl portrait
	...
	m_pgb_health = new ProgressBar(m_pnl_portrait,
		...
		m_val->UITextures[CUSTIMAGE_PGBTEXT_HEALTH], m_val->UITextures[CUSTIMAGE_PGBTEXT_HEALTH_BACK],
	...
	m_pgb_mana = new ProgressBar(m_pnl_portrait,
		...
		m_val->UITextures[CUSTIMAGE_PGBTEXT_MANA], m_val->UITextures[CUSTIMAGE_PGBTEXT_MANA_BACK],
	...
	m_pgb_exp = new ProgressBar(m_pnl_portrait,
		...
		m_val->UITextures[CUSTIMAGE_PGBTEXT_EXP], m_val->UITextures[CUSTIMAGE_PGBTEXT_EXP_BACK],
	...
	m_lbl_health = new Label(m_pnl_portrait, Vector2i(LBL_HEALTH_POSITION_X, LBL_HEALTH_POSITION_Y), m_val->FontTextures[TEXTCOLOR_RED], "", 
	...
	// Label de mana
	m_lbl_mana = new Label(m_pnl_portrait, Vector2i(LBL_MANA_POSITION_X, LBL_MANA_POSITION_Y), m_val->FontTextures[TEXTCOLOR_BLUE], "", 
	...
	// Label d'exp
	m_lbl_exp = new Label(m_pnl_portrait, Vector2i(LBL_EXP_POSITION_X, LBL_EXP_POSITION_Y), m_val->FontTextures[TEXTCOLOR_YELLOW], "", 
	...
	m_pnl_playerImage = new Panel(m_pnl_portrait, 
		...
		m_val->UITextures[CUSTIMAGE_PORTRAIT_MALE], 1, PB_PORTRAIT_NAME);
	m_pnl_portrait->AddControl(m_pnl_playerImage);

#pragma region Enfants de m_pnl_playerImage

	m_lbl_playerLevel = new Label(m_pnl_playerImage, Vector2i(), m_val->FontTextures[TEXTCOLOR_BLUE], "", 
...
#pragma endregion
	...
	m_pgb_energy = new ProgressBar(m_pnl_playscreen,
		...
		m_val->UITextures[CUSTIMAGE_PGBTEXT_ENERGY], m_val->UITextures[CUSTIMAGE_PGBTEXT_ENERGY_BACK],
	...
	//Label d'énergie
	m_lbl_energy = new Label(m_pnl_playscreen, Vector2i(LBL_ENERGY_POSITION_X, LBL_ENERGY_POSITION_Y), m_val->FontTextures[TEXTCOLOR_GREEN], "", 
...
#pragma region Enfants de m_pnl_time
	...
	m_lbl_time = new Label(m_pnl_time, Vector2i(0,0), m_val->FontTextures[TEXTCOLOR_WHITE], "", Label::TEXTDOCK_MIDDLECENTER, false, 
