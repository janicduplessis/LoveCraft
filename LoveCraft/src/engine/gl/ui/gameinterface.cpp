...
#pragma region Proprietes des controles
	...
	if (val.Character->HealthPerc() <= PGB_HEALTH_LOW_TRESHOLD && pgb_health->IsForeground(CUSTIMAGE_PGBTEXT_HEALTH))
		pgb_health->SetForeground(CUSTIMAGE_PGBTEXT_HEALTH_LOW);
	else if (val.Character->HealthPerc() > PGB_HEALTH_LOW_TRESHOLD && pgb_health->IsForeground(CUSTIMAGE_PGBTEXT_HEALTH_LOW))
		pgb_health->SetForeground(CUSTIMAGE_PGBTEXT_HEALTH);
...
void GameInterface::Init(const ValuesGameInterface& val)
{
	...
	pnl_screen->InitLocalizable(Point(), Size(val.Width, val.Height), CUSTIMAGE_NONE, 0);
...
#pragma region Enfants de Main
	...
	pnl_playscreen->InitLocalizable(Point(INTERFACE_SIDE_LEFT_WIDTH, INTERFACE_BOTTOM_HEIGHT),
		...
		pnl_screen->GetSize().h - INTERFACE_BOTTOM_HEIGHT - INTERFACE_TOP_HEIGHT), CUSTIMAGE_NONE, pnl_screen);
...
#pragma region Enfants de Playscreen
	...
	lbx_infos->InitLocalizable(Point(pnl_playscreen->GetSize().w - 250, pnl_playscreen->GetSize().h - 64 - 26 * (2 + 15)), Size(), CUSTIMAGE_NONE, pnl_playscreen);
	lbx_infos->InitTextual(TEXTCOLOR_RED, false, 15.f, 15.f, 0.5f);
	...
	lbx_console->InitLocalizable(Point(pnl_playscreen->GetSize().w - 64 - LB_CONSOLE_SIZE_W, TXB_CONSOLE_SIZE_H + 5), 
		Size(LB_CONSOLE_SIZE_W, TXB_CONSOLE_SIZE_H), CUSTIMAGE_CONSOLE_BACK, pnl_playscreen);
	...
	lbx_console->InitListBox(LB_CONSOLE_LINE_NUMBER, LB_CONSOLE_LINE_GAP, Point(LB_CONSOLE_BODER_OFFSET_S, LB_CONSOLE_BODER_OFFSET_B),
		true, CUSTIMAGE_ARROWBUTTON_UP, CUSTIMAGE_ARROWBUTTON_DOWN);
	...
	txb_console->InitLocalizable(Point(pnl_playscreen->GetSize().w - 64 - LB_CONSOLE_SIZE_W, 0),
		Size(TXB_CONSOLE_SIZE_W, 40), CUSTIMAGE_CONSOLE_TEXTBOX_BACK, pnl_playscreen);
	txb_console->InitTextual(THEME_CONSOLE);
	txb_console->Text->SetDocking(TEXTDOCK_MIDDLELEFT);
	txb_console->SetColor(TEXTCOLOR_WHITE);
	txb_console->Text->SetOffset(Point(TXB_CONSOLE_OFFSET_X, 0));
	txb_console->Hide();
	// Frame portrait
	...
	pnl_portrait->InitLocalizable(Point(PNL_PORTRAIT_POSITION_X, PNL_PORTRAIT_POSITION_Y),
		Size(PNL_PORTRAIT_SIZE_W, PNL_PORTRAIT_SIZE_H), CUSTIMAGE_PORTRAIT_FRAME, pnl_playscreen);
...
#pragma region Enfants pnl portrait
	...
	pgb_health->InitLocalizable(Point(PGB_HEALTH_POSITION_X, PGB_HEALTH_POSITION_Y),
		Size(PGB_HEALTH_SIZE_W, PGB_HEALTH_SIZE_H), CUSTIMAGE_PGBTEXT_HEALTH_BACK, pnl_portrait);
	pgb_health->Init(ProgressBar::BARMODE_HORIZONTAL_LTR, CUSTIMAGE_PGBTEXT_HEALTH, PGB_HEALTH_BORDER_SIZE);
	// Barre de mana
	...
	pgb_mana->InitLocalizable(Point(PGB_MANA_POSITION_X, PGB_MANA_POSITION_Y),
		Size(PGB_MANA_SIZE_W, PGB_MANA_SIZE_H), CUSTIMAGE_PGBTEXT_MANA_BACK, pnl_portrait);
	pgb_mana->Init(ProgressBar::BARMODE_HORIZONTAL_LTR, CUSTIMAGE_PGBTEXT_MANA, PGB_MANA_BORDER_SIZE);
	// Barre d'expérience
	...
	pgb_exp->InitLocalizable(Point(PGB_EXP_POSITION_X, PGB_EXP_POSITION_Y),
		Size(PGB_EXP_SIZE_W, PGB_EXP_SIZE_H), CUSTIMAGE_PGBTEXT_EXP_BACK, pnl_portrait);
	pgb_exp->Init(ProgressBar::BARMODE_HORIZONTAL_LTR, CUSTIMAGE_PGBTEXT_EXP, PGB_EXP_BORDER_SIZE);
	// Label de vie
	...
	lbl_plrHealth->SetColor(TEXTCOLOR_RED);
	// Label de mana
	...
	lbl_plrMana->SetColor(TEXTCOLOR_BLUE);
	// Label d'exp
	...
	lbl_plrExp->SetColor(TEXTCOLOR_YELLOW);
	...
	pnl_playerImage->InitLocalizable(Point(PB_PORTRAIT_POSITION_X, PB_PORTRAIT_POSITION_Y),
		Size(PB_PORTRAIT_SIZE_W, PB_PORTRAIT_SIZE_H), CUSTIMAGE_PORTRAIT_MALE, pnl_portrait);
	...
	lbl_playerLevel->InitTextual(TEXTCOLOR_YELLOW, false, LBL_PLAYER_LEVEL_H, LBL_PLAYER_LEVEL_W, LBL_PLAYER_LEVEL_I);
	lbl_playerLevel->Init(TEXTDOCK_TOPLEFT, Point(0, -5));
	lbl_playerLevel->SetMsg("L: &var");
	...
	lbl_crntBT->InitTextual(TEXTCOLOR_BLUE, false, 
	...
	pb_crntBT->InitLocalizable(Point(lbl_crntBT->GetPosition().x + 84, 
		lbl_crntBT->GetPosition().y), Size(60,60), CUSTIMAGE_NONE, pnl_playscreen);
...
#pragma endregion
	...
	pgb_energy->InitLocalizable(Point(PGB_ENERGY_POSITION_X, PGB_ENERGY_POSITION_Y),
		Size(PGB_ENERGY_SIZE_W, PGB_ENERGY_SIZE_H), CUSTIMAGE_PGBTEXT_ENERGY_BACK, pnl_playscreen);
	pgb_energy->Init(ProgressBar::BARMODE_VERTICAL_DTU, CUSTIMAGE_PGBTEXT_ENERGY, PGB_ENERGY_BORDER_SIZE);
	//Label d'énergie
	...
	lbl_plrEnergy->InitTextual(TEXTCOLOR_GREEN, LBL_ENERGY_ITALIC, LBL_ENERGY_CHAR_H, LBL_ENERGY_CHAR_W, LBL_ENERGY_CHAR_I);
...
#pragma region Controles de debug
	...
	lbl_plrPos->InitTextual(TEXTCOLOR_GREEN, LBL_GENERIC_ITALIC, LBL_GENERIC_CHAR_H, LBL_GENERIC_CHAR_W, LBL_GENERIC_CHAR_I);
	...
	lbl_plrSpd->InitTextual(TEXTCOLOR_BLUE, LBL_GENERIC_ITALIC, LBL_GENERIC_CHAR_H, LBL_GENERIC_CHAR_W, LBL_GENERIC_CHAR_I);
	...
	lbl_plrAcc->InitTextual(TEXTCOLOR_RED, LBL_GENERIC_ITALIC, LBL_GENERIC_CHAR_H, LBL_GENERIC_CHAR_W, LBL_GENERIC_CHAR_I);
	...
	lbl_mousePos->InitTextual(TEXTCOLOR_WHITE, LBL_GENERIC_ITALIC, LBL_GENERIC_CHAR_H, LBL_GENERIC_CHAR_W, LBL_GENERIC_CHAR_I);
	...
	lbl_FPS->InitTextual(TEXTCOLOR_YELLOW, LBL_GENERIC_ITALIC, LBL_GENERIC_CHAR_H, LBL_GENERIC_CHAR_W, LBL_GENERIC_CHAR_I);
	...
	m_controlmousetest->InitTextual(TEXTCOLOR_YELLOW, LBL_GENERIC_ITALIC, LBL_GENERIC_CHAR_H, LBL_GENERIC_CHAR_W, LBL_GENERIC_CHAR_I);
...
#pragma endregion
	...
	pnl_time->InitLocalizable(Point(pnl_playscreen->GetSize().w - 128, pnl_playscreen->GetSize().h - 64), 
		Size(128, 64),CUSTIMAGE_CLOCK_BG, pnl_playscreen);
...
>>>>>>> 71ff4f5479c60d194183fd195bd439a9e1ec319b
	return false;
}
