...
#pragma region Proprietes des controles
	...
	if (m_lb_infos->GP(Control::PROPBOL_VISIBLE) && !Info::Get().Options().GetOptInfos())
		m_lb_infos->SP(Control::PROPBOL_VISIBLE, false);
	else if (!m_lb_infos->GP(Control::PROPBOL_VISIBLE) && Info::Get().Options().GetOptInfos())
		m_lb_infos->SP(Control::PROPBOL_VISIBLE, true);
	//Change la texture de la barre de vie en fonction du %. Ne réassigne la texture que si on en a besoin
	if (val.Character->HealthPerc() <= PGB_HEALTH_LOW_TRESHOLD && m_pgb_health->GP(Control::PROPTEXT_BAR) == val.UITextures[CUSTIMAGE_PGBTEXT_HEALTH])
		m_pgb_health->SP(Control::PROPTEXT_BAR, val.UITextures[CUSTIMAGE_PGBTEXT_HEALTH_LOW]);
	else if (val.Character->HealthPerc() > PGB_HEALTH_LOW_TRESHOLD && m_pgb_health->GP(Control::PROPTEXT_BAR) == val.UITextures[CUSTIMAGE_PGBTEXT_HEALTH_LOW])
		m_pgb_health->SP(Control::PROPTEXT_BAR, val.UITextures[CUSTIMAGE_PGBTEXT_HEALTH]);
	...
	{
		m_pgb_energy->SP(Control::PROPBOL_VISIBLE, true);
		m_lbl_energy->SP(Control::PROPBOL_VISIBLE, true);
	}
	else if (val.Character->Energy() == val.Character->EnergyMax())
	{
		m_pgb_energy->SP(Control::PROPBOL_VISIBLE, false);
		m_lbl_energy->SP(Control::PROPBOL_VISIBLE, false);
	}
	// Controles de debug
	if (Info::Get().Options().GetOptDebug() && !m_lbl_FPS->GP(Control::PROPBOL_VISIBLE))
	{
		m_lbl_plrPos->SP(Control::PROPBOL_VISIBLE, true);
		m_lbl_plrSpd->SP(Control::PROPBOL_VISIBLE, true);
		m_lbl_plrAcc->SP(Control::PROPBOL_VISIBLE, true);
		m_lbl_mousePos->SP(Control::PROPBOL_VISIBLE, true);
		m_lbl_FPS->SP(Control::PROPBOL_VISIBLE, true);
	}
	else if (!Info::Get().Options().GetOptDebug() && m_lbl_FPS->GP(Control::PROPBOL_VISIBLE))
	{
		m_lbl_plrPos->SP(Control::PROPBOL_VISIBLE, false);
		m_lbl_plrSpd->SP(Control::PROPBOL_VISIBLE, false);
		m_lbl_plrAcc->SP(Control::PROPBOL_VISIBLE, false);
		m_lbl_mousePos->SP(Control::PROPBOL_VISIBLE, false);
		m_lbl_FPS->SP(Control::PROPBOL_VISIBLE, false);
	}
...
void GameInterface::TextUpdate(const ValuesGameInterface& val)
{
	...
	m_lbl_health->SP(Label::PropString::PROPSTR_TEXT, ss.str());
	ss.str("");
	//Energie
	ss << (int)val.Character->Energy() << " / " << (int)val.Character->EnergyMax();
	m_lbl_energy->SP(Label::PropString::PROPSTR_TEXT, ss.str());
	ss.str("");
	//Mana
	ss << (int)val.Character->Mana() << " / " << (int)val.Character->ManaMax();
	m_lbl_mana->SP(Label::PropString::PROPSTR_TEXT, ss.str());
	ss.str("");
	//Experience
	ss << (int)val.Character->Exp() << " / " << (int)val.Character->ExpNext();
	m_lbl_exp->SP(Label::PropString::PROPSTR_TEXT, ss.str());
	ss.str("");
	//Niveau
	ss << val.Character->Level();
	m_lbl_playerLevel->SP(Label::PropString::PROPSTR_TEXT, ss.str());
	ss.str("");
	//Position
	...
	m_lbl_plrPos->SP(Label::PropString::PROPSTR_TEXT, ss.str());
	ss.str("");
	//Vitesse
	ss << "Vitesse :      " << val.Player->Speed();
	m_lbl_plrSpd->SP(Label::PropString::PROPSTR_TEXT, ss.str());
	ss.str("");
	//Accélération
	ss << "Acceleration : " << val.Player->Acceleration();
	m_lbl_plrAcc->SP(Label::PropString::PROPSTR_TEXT, ss.str());
	ss.str("");
	//Psotion souris
	ss << "Pos. Souris :  " << val.Mousepos;
	m_lbl_mousePos->SP(Label::PropString::PROPSTR_TEXT, ss.str());
	ss.str("");
	//FPS
	ss << "Fps :          " << std::setprecision(2) << val.Fps;
	m_lbl_FPS->SP(Label::PropString::PROPSTR_TEXT, ss.str());
	ss.str("");
	//Heure
	time_t currentTime;
	time (&currentTime);
	...
	m_lbl_time->SP(Label::PropString::PROPSTR_TEXT, ss.str());
	ss.str("");
}
...
#pragma region Enfants de Main
	...
	m_pnl_playscreen->CtrlInit(m_pnl_screen, 
		...
		Vector2i(m_pnl_screen->GP(Control::PROPVCT2_SIZE).x - INTERFACE_SIDE_LEFT_WIDTH - INTERFACE_SIDE_RIGHT_WIDTH, 
		m_pnl_screen->GP(Control::PROPVCT2_SIZE).y - INTERFACE_BOTTOM_HEIGHT - INTERFACE_TOP_HEIGHT),
		0, "playscreen");
...
#pragma region Enfants de Playscreen
	...
	// Panel welcome
	//m_pnl_welcome = new Panel(m_pnl_playscreen, Vector2i(m_pnl_playscreen->GP(Control::PROPVCT2_SIZE).x / 2 - 300, 
	//	m_pnl_playscreen->GP(Control::PROPVCT2_SIZE).y - 300), Vector2i(600, 200), val.UITextures[CUSTIMAGE_MENU_BACKGROUND],
	//	2, "welcomepanel");
	//m_pnl_playscreen->AddControl(m_pnl_welcome);
	//m_pnl_welcome->SP(Control::PROPBOL_REPEATTEXTURE, false);
	...
	//m_pb_welcomeface->SP(Control::PROPBOL_REPEATTEXTURE, false);
	...
	m_lb_infos->CtrlInit(m_pnl_playscreen, Vector2i(5, 
		m_pnl_playscreen->GP(Control::PROPVCT2_SIZE).y - LBL_GENERIC_CHAR_H*6 - 26*12),
		Vector2i(), 0, "lb_infos");
	...
	m_txb_console->CtrlInit(m_pnl_playscreen, 
		Vector2i(m_pnl_playscreen->GP(Control::PROPVCT2_SIZE).x - 64 - (int)LB_CONSOLE_SIZE_W, 0),
...
#pragma region Enfants pnl portrait
	...
	m_lbl_health->Init(Label::TEXTDOCK_NONE, Vector2f());
	...
	m_lbl_mana->Init(Label::TEXTDOCK_NONE, Vector2f());
	...
	m_lbl_exp->Init(Label::TEXTDOCK_NONE, Vector2f());
...
#pragma region Enfants de m_pnl_playerImage
	...
	m_lbl_playerLevel->Init(Label::TEXTDOCK_TOPCENTER, Vector2f(0, -10));
...
#pragma endregion
	...
	m_lbl_energy->Init(Label::TEXTDOCK_NONE, Vector2f());
...
#pragma region Controles de debug
	...
	m_lbl_plrPos->CtrlInit(m_pnl_playscreen, Vector2i(5, m_pnl_playscreen->GP(Panel::PROPVCT2_SIZE).y - LBL_GENERIC_CHAR_H),
		Vector2i(), 0, "pos");
	...
	m_lbl_plrPos->Init(Label::TEXTDOCK_NONE, Vector2f());
	m_pnl_playscreen->AddControl(m_lbl_plrPos);
	//Label Vitesse
	m_lbl_plrSpd = new Label();
	m_lbl_plrSpd->CtrlInit(m_pnl_playscreen, Vector2i(5, m_pnl_playscreen->GP(Panel::PROPVCT2_SIZE).y - LBL_GENERIC_CHAR_H * 2),
		Vector2i(), 0, "spd");
	...
	m_lbl_plrSpd->Init(Label::TEXTDOCK_NONE, Vector2f());
	m_pnl_playscreen->AddControl(m_lbl_plrSpd);
	//Label Acceleration
	m_lbl_plrAcc = new Label();
	m_lbl_plrAcc->CtrlInit(m_pnl_playscreen, Vector2i(5, m_pnl_playscreen->GP(Panel::PROPVCT2_SIZE).y - LBL_GENERIC_CHAR_H * 3),
		Vector2i(), 0, "acc");
	...
	m_lbl_plrAcc->Init(Label::TEXTDOCK_NONE, Vector2f());
	m_pnl_playscreen->AddControl(m_lbl_plrAcc);
	//Label mouse position
	m_lbl_mousePos = new Label();
	m_lbl_mousePos->CtrlInit(m_pnl_playscreen, Vector2i(5, m_pnl_playscreen->GP(Panel::PROPVCT2_SIZE).y - LBL_GENERIC_CHAR_H * 4),
		Vector2i(), 0, "mpos");
	...
	m_lbl_mousePos->Init(Label::TEXTDOCK_NONE, Vector2f());
	m_pnl_playscreen->AddControl(m_lbl_mousePos);
	//Label FPS
	m_lbl_FPS = new Label();
	m_lbl_FPS->CtrlInit(m_pnl_playscreen, Vector2i(5, m_pnl_playscreen->GP(Panel::PROPVCT2_SIZE).y - LBL_GENERIC_CHAR_H * 5),
		Vector2i(), 0, "fps");
	...
	m_lbl_FPS->Init(Label::TEXTDOCK_NONE, Vector2f());
...
#pragma endregion
	...
	m_pnl_time->CtrlInit(m_pnl_playscreen, Vector2i(m_pnl_playscreen->GP(Panel::PROPVCT2_SIZE).x - 128, m_pnl_playscreen->GP(Panel::PROPVCT2_SIZE).y - 64), 
...
#pragma region Enfants de m_pnl_time
	...
	m_lbl_time->Init(Label::TEXTDOCK_MIDDLECENTER, Vector2f());
...
bool GameInterface::MousePressEvent( const MOUSE_BUTTON &button, int x, int y )
{
	...
	Vector2i& size = m_lb_console->GP(Control::PROPVCT2_SIZE);
	Vector2i& play = m_pnl_screen->GP(Control::PROPVCT2_SIZE);
	switch (button)
	{
	...
	case MOUSE_BUTTON_LEFT:
		if (m_lb_console->MousePressEvents(x, m_pnl_playscreen->GP(Control::PROPVCT2_SIZE).y - y))
			return true;
		break;
...
bool GameInterface::TextenteredEvent( unsigned int val )
{
	...
	if (val == 13 && !m_txb_console->HasFocus())
	{
		...
		m_txb_console->SP(Control::PROPBOL_VISIBLE, true);
		return true;
	}
