...
{
	//Fond d'écran
	m_menu_screen = new Panel(0, Vector2i(), Vector2i(m_val->GetWidth(), m_val->GetHeight()), m_val->GetTexture(CUSTIMAGE_MENU_BACKGROUND), 1, "menu_main");
	m_menu_screen->SetRepeatTexture(false);

	//Loading screen
	m_menu_loading = new PictureBox(0, Vector2i(0, 0), m_menu_screen->Size(), m_val->GetTexture(CUSTIMAGE_LOADING_SCREEN), "loading");
	...
	m_menu_panel = new Panel(m_menu_screen, 
		Vector2i(m_val->GetWidth() / 2 - MENU_PANEL_SIZE_X / 2, m_val->GetHeight() / 2 - MENU_PANEL_SIZE_Y / 2), 
		Vector2i(MENU_PANEL_SIZE_X, MENU_PANEL_SIZE_Y), m_val->GetTexture(CUSTIMAGE_MENU_MAIN_WINDOW), 2, MENU_PANEL_NAME);
	...
	m_menu_logo = new PictureBox(m_menu_panel, 
		...
		Vector2i(controlWidth, MENU_LOGO_SIZE_Y), m_val->GetTexture(CUSTIMAGE_MENU_LOGO), "logo");
	...
	m_menu_controls = new Panel(m_menu_panel,
		...
		m_val->GetTexture(CUSTIMAGE_MENU_MAIN_WINDOW), 3, "menu_controls");
	...
	m_menu_fullscreen = new Button(m_menu_controls,
		...
		m_val->GetTexture(CUSTIMAGE_MENU_BUTTON_BACK), m_val->GetFont(TEXTCOLOR_YELLOW),
