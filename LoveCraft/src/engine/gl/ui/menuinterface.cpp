...
void MenuInterface::Init(const ValuesInterface& val)
{
	...
	m_menu_screen->InitLocalizable(Point(), Size(val.Width, val.Height), val.UITextures[CUSTIMAGE_MENU_BACKGROUND], 0);

	//Loading screen
	m_menu_loading = new PictureBox();
	m_menu_loading->InitControl("loading");
	m_menu_loading->InitLocalizable(Point(), m_menu_screen->GetSize(), val.UITextures[CUSTIMAGE_LOADING_SCREEN], m_menu_screen);
	m_menu_loading->Hide();
	m_menu_screen->AddControl(m_menu_loading);

	//Paneau principal du menu
	m_menu_panel = new Panel();
	m_menu_panel->InitControl(MENU_PANEL_NAME);
	m_menu_panel->InitLocalizable(Point(val.Width / 2 - MENU_PANEL_SIZE_X / 2, val.Height / 2 - MENU_PANEL_SIZE_Y / 2), 
		Size(MENU_PANEL_SIZE_X, MENU_PANEL_SIZE_Y), val.UITextures[CUSTIMAGE_MENU_MAIN_WINDOW], m_menu_screen);
	...
	m_menu_fullscreen->InitControl(MENU_BUTTON_START_FULL_NAME);
	m_menu_fullscreen->InitLocalizable(Point(buttonPosX, buttonPosX + MENU_BUTTONS_SIZE_Y * 3 - MENU_BUTTONS_INTERVAL),
		Size(buttonWidth, MENU_BUTTONS_SIZE_Y), val.UITextures[CUSTIMAGE_MENU_BUTTON_BACK], m_menu_controls);
	...
	m_menu_start->InitControl(MENU_BUTTON_DEBUG);
	...
	m_menu_close->InitControl(MENU_BUTTON_CLOSE);
